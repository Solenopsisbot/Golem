...
export const BUILTINS: BuiltinSpec[] = [
  ...
  { name: "require_dimension", params: [p("name", "string")], returns: "string", doc: "Fail immediately unless you are in this dimension (\"the_end\", \"overworld\", \"the_nether\"). Put it at the top of any script holding hardcoded coordinates: the three worlds share a coordinate space, and dying moves you between them, so a plan written in one runs happily and wrongly in another.", blocking: false },
  { name: "pearl_to", params: [p("pos", "pos")], returns: "record", doc: "Throw an ender pearl at a position and land there, solving the throw by simulation (a pearl is slower and floatier than an arrow, so bow instincts do not transfer). Costs 5 damage on landing - cheap against standing in dragon breath, not free. Also the fastest way across a gap. Returns .pitch .yaw .ticks", blocking: true },
  { name: "shoot_static", params: [p("target", "any"), p("shots", "int", "1")], returns: "record", doc: "Bow something that does NOT move - an end crystal, a spawner. Solves the launch angle by simulating the arrow (drag included), so it stays right at any range, where the body's shoot_at is about half the drop it needs and buries arrows short. Aims a block above the target's position, which is its feet. Returns .fired .pitch", blocking: true },
