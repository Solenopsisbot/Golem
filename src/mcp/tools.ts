...
export const SHEM_TOOLS: ToolDef[] = [
  ...
  def({ name: "shem_run", description: "Run a script from a file (path under your workspace, or lib/<name>). Blocks until it ends or timeout_s passes (the run keeps going in that case; use shem_status/shem_wait). background: true returns immediately. interrupt: true preempts a lower-priority foreground run.", input: { path: z.string(), script: z.string().optional(), params: paramsShape, priority: z.number().int().min(0).max(100).default(50), interrupt: z.boolean().default(false), background: z.boolean().default(false), timeout_s: z.number().min(1).max(3600).default(120) },
    async run(a, tc) {
      const e = needShem(tc);
